...
async function run() {
  ...
  // Home
  xml += `  <url><loc>${BASE_URL}</loc><lastmod>${staticLastMod}</lastmod><priority>1.0</priority></url>\n`;

  // Static Pages
  xml += `  <url><loc>${BASE_URL}/piet</loc><lastmod>${staticLastMod}</lastmod><priority>0.8</priority></url>\n`;
  xml += `  <url><loc>${BASE_URL}/reed</loc><lastmod>${staticLastMod}</lastmod><priority>0.8</priority></url>\n`;
  xml += `  <url><loc>${BASE_URL}/zakelijk</loc><lastmod>${staticLastMod}</lastmod><priority>0.8</priority></url>\n`;
  xml += `  <url><loc>${BASE_URL}/prijzen</loc><lastmod>${staticLastMod}</lastmod><priority>0.7</priority></url>\n`;
  xml += `  <url><loc>${BASE_URL}/contact</loc><lastmod>${staticLastMod}</lastmod><priority>0.5</priority></url>\n`;
  xml += `  <url><loc>${BASE_URL}/privacy</loc><lastmod>${staticLastMod}</lastmod><priority>0.3</priority></url>\n`;

  // Thema pages
  const themes = ["bbq-weer", "strandweer", "hardloopweer", "hooikoorts", "wintersport-nl"];
  themes.forEach(slug => {
    xml += `  <url><loc>${BASE_URL}/weer/themas/${slug}</loc><lastmod>${staticLastMod}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>\n`;
  });

  // Weer subpages
  xml += `  <url><loc>${BASE_URL}/weer/48-uur</loc><lastmod>${staticLastMod}</lastmod><priority>0.6</priority></url>\n`;
  ...
  [...priorityPlaces, ...otherPlaces].forEach((place: any) => {
    ...
    xml += `  <url><loc>${BASE_URL}/weer/${place.province}/${slug}</loc><lastmod>${placesLastMod}</lastmod><changefreq>hourly</changefreq><priority>${priorityCityNames.includes(place.name) ? '0.7' : '0.5'}</priority></url>\n`;
  });
