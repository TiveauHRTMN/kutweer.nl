...
function OnboardingScreen({ nav }) {
  ...
  const steps = [
    {
      title: "Waar ben je?",
      sub: "We gebruiken je locatie om het weerbericht op jou af te stemmen. Je kunt er later meer toevoegen.",
      body: (
        <>
          <TextField label="Jouw stad of dorp" value={location} onChange={setLocation} placeholder="Bijv. Utrecht" autoFocus />
          <div style={{display:'flex', gap:8, flexWrap:'wrap', marginTop: 4}}>
            {['Amsterdam','Rotterdam','Utrecht','Den Haag','Groningen','Eindhoven'].map(c => (
              <button key={c} className={`btn btn-sm ${location===c?'btn-soft':'btn-ghost'}`} onClick={() => setLocation(c)}>{c}</button>
            ))}
          </div>
        </>
      )
