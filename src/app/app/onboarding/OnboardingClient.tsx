...
export default function OnboardingClient({ email }: { email: string }) {
  ...
  async function persistAndGo(nextHref: string) {
    ...
    try {
      ...
      // Onderwerpen + meldingstijd: in user_metadata. Wordt bij /checkout
  ...
  async function handleFinish() {
    // Als gebruiker al een tier had gekozen op /prijzen: door naar checkout.
          ...
          <div className="flex gap-1.5">
            {stepTitles.map((_, i) => (
              <div
                ...
                style={{
                  background: i <= step ? "var(--wz-brand)" : "var(--ink-200)",
                }}
              />
            ))}
          </div>
