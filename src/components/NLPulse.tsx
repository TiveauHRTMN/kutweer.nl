    ...
    <div className="relative group overflow-hidden bg-white/5 border-y border-white/5 h-5 flex items-center" style={{ isolation: "isolate" }}>
      ...
      <div className="absolute left-0 top-0 bottom-0 z-20 px-2 flex items-center gap-1 bg-gradient-to-r from-white/10 via-white/5 to-transparent pr-6 pointer-events-none">
        <div className="relative flex">
          <Radio className="w-2 h-2 text-accent-orange animate-pulse" />
          <div className="absolute inset-0 bg-accent-orange/40 rounded-full blur-[2px] animate-pulse" />
        </div>
        <span className="text-[8px] font-black uppercase tracking-[0.1em] text-white whitespace-nowrap opacity-80">
          Live <span className="hidden xs:inline">Pulse</span>
        </span>
      </div>

      {/* Scrolling Content - Brighter text for readability on light glass */}
      <div className="flex gap-6 sm:gap-8 text-[8px] font-extrabold uppercase tracking-widest text-white/90 animate-marquee hover:pause-marquee pl-12 sm:pl-20">
