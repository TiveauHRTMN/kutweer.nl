...
import { getDEStationsWeather } from "@/app/actions";
...
export default function DEPulse() {
  const [stations, setStations] = useState<Array<{ name: string; temp: number; weatherCode: number; isDay: boolean }>>([]);

  useEffect(() => {
    getDEStationsWeather().then(data => {
      setStations([...data].sort((a, b) => a.name.localeCompare(b.name, "de")));
    });
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          width: "max-content",
          animation: "depulse-marquee 110s linear infinite",
          paddingLeft: "5rem",
        }}
      >
        {items.map((s, i) => (
