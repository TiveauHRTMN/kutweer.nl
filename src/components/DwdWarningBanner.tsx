...
const SEVERITY_STYLE: Record<DWDSeverity, { bg: string; border: string; chip: string; chipBg: string }> = {
  ...
  VIOLET: { bg: "bg-fuchsia-700/25", border: "border-fuchsia-400/70", chip: "text-fuchsia-100", chipBg: "bg-fuchsia-700/60" },
};

interface Props {
  warnings: DWDWarning[];
  /** Toon de "Details"-link naar /de/warnungen. Default true. */
  detailsLink?: boolean;
  /** Compact = enkel chip + 1 regel, geen description of adviesblokken. */
  compact?: boolean;
}

export default function DwdWarningBanner({ warnings, detailsLink = true, compact = false }: Props) {
  const active = warnings.filter((w) => w.severity !== "GREEN");
  if (active.length === 0) return null;

  const ordered = [...active].sort((a, b) => {
    const order: Record<DWDSeverity, number> = { VIOLET: 0, RED: 1, ORANGE: 2, YELLOW: 3, GREEN: 4 };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                    ...
                    <span className="text-[11px] font-medium text-white/60">{warning.region}</span>
                    {window && (
                      <span className="text-[11px] font-bold text-white/80">· {window}</span>
                    )}
                  </div>
                  {!compact && warning.headline && (
